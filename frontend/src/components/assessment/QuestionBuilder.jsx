...
export default function QuestionBuilder({ assessmentId }) {
  ...
  const handleFetchFromCodewars = async () => {
    ...
    const kata = await assessmentService.fetchCodewarsKata();
    setPrompt(kata.prompt);
  ...
  const handleAdd = () => {
    ...
    dispatch(
      addQuestionToAssessment({
        assessmentId,
        question: { type, prompt, source: "manual" },
      })
    );
    setPrompt("");
  };
  ...
  return (
    <div>
      <div className="mb-3.5">
        <label className="mb-1.5 block text-xs font-semibold text-ink">Add question — type</label>
      ...
      {type === "kata" && (
        <Button
          ...
          className="mb-3.5"
        >
          {fetchingKata ? "Fetching…" : "⟳ Fetch from Codewars API"}
        </Button>
      )}
